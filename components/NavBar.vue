<template>
  <div class="sticky top-0 z-30 bg-skin-light dark:bg-skin-dark">
    <nav ref="navelement"
      ...
      <div class="flex justify-between">
        <div class="w-1/6 z-40 relative flex flex-row items-center flex-nowrap">
          ...
          <button @click="toggle('language')" class="font-medium rounded-lg text-sm text-center inline-flex items-centent"
            type="button">
          ...
          <div v-show="showDropdown" class="fixed top-12">
            <div class="relative z-50 border-2 w-44 bg-skin-light dark:bg-skin-dark rounded shadow">
        ...
        <div class="flex-grow content-center text-center align-middle">
          <a :href="homeLink" class="inline-block">
            <img :src="$props.logo" class="h-7 sm:h-10 m-1" />
          </a>
        </div>
    ...
    <transition class="z-30" enter-from-class="translate-y-[150%] opacity-0" enter-active-class="transition duration-400"
      ...
      <div v-if="showMenu" class="fixed top-14 h-screen w-screen bg-skin-light dark:bg-skin-dark">
        <div
          class="pt-10 bg-skin-light dark:bg-skin-dark text-skin-base dark:text-skin-dark  overflow-y-scroll shadow-md">
          <!-- v-show="show" -->
