<template>
  <div>
    <div v-if="countPages > 1" class="flex items-center justify-between border-1 px-4 py-3">
      <div data-testid="ListPaginator.sm" class="flex flex-1 justify-between sm:hidden">
        <button @click="previous" data-testid="ListPaginator.sm_previous"
          class="inline-flex items-center rounded-md px-4 py-2 text-xs font-medium text-skin-base dark:text-skin-dark"
          :class="getClass('previous')" :disabled="selectedPage <= 1">
          {{ $t('paginator.previous') }}
        </button>
        <p class="text-xs tracking-widest">
          {{ $t('paginator.statusshort', [selectedPage, countPages]) }}
        </p>
        <button @click="next" data-testid="ListPaginator.sm_next"
          class="inline-flex items-center rounded-md px-4 py-2 text-xs font-medium text-skin-base dark:text-skin-dark"
          :class="getClass('next')" :disabled="selectedPage >= countPages">
          {{ $t('paginator.next') }}
        </button>
      </div>
      <div class="hidden sm:pt-6 sm:flex sm:flex-1 sm:items-center sm:justify-between">
        ...
        <div>
          <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
            <button :class="getClass('previous')" :disabled="selectedPage <= 1" data-testid="ListPaginator.lg_previous"
              class="relative inline-flex items-center text-sm font-medium focus:z-20 px-2 py-2" @click="previous">
            ...
            <div v-for="page in pagesList" :key="page">
              ...
              <div v-else>
                <button data-testid="ListPaginator.setpage"
                  class="relative w-8 inline-flex items-center text-sm focus:z-20 px-2 py-2" :class="getClass(page)"
                  @click="(typeof page == 'number' ? setPage(page) : '')">{{ page }}</button>
              </div>
            </div>
            <button :class="getClass('next')" :disabled="selectedPage >= countPages" data-testid="ListPaginator.lg_next"
              class="relative inline-flex items-center text-sm font-medium focus:z-20 px-2 py-2" @click="next">
