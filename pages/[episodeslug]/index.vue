  ...
  <div>
    <div class="flex flex-col items-center">
      <div class="w-5/6 md:w-2/3 flex flex-row justify-end">
        <NuxtLink class="text-orange-500 text-xs" v-if="user" :to="'/admin/' + slug">
          {{$t("episode.edit_short")}}
        </NuxtLink>
        <div v-else class="pt-14"></div>
      </div>
      <div class="w-5/6 md:w-2/3 md:h-60 flex flex-row">
        <img class="h-28 md:h-60 w-28 md:w-60 shrink-0" :src="episode.image" />
        <div class="pl-6 md:pl-12 pt-2 pb-8 flex flex-col justify-around rounded-r-md">
          <div
            class="text-xs md:text-sm tracking-wide uppercase bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 via-orange-700 to-orange-900 font-bold"
          >
            {{ serie.title }}
          </div>
          ...
          <div class="text-xs md:text-sm tracking-wide text-gray-500">{{ episode.subtitle }}</div>
        </div>
          <div class="text-xs md:text-sm tracking-wider">{{ episode.creator }}</div>
    ...
    <div class="flex flex-col items-center">
      <div class="w-5/6 md:w-2/3 pt-6">
        <audio-player class="text-white bg-black" :file="episode.link"></audio-player>
      </div>
        ...
        <div class="md:w-2/3 pt-6 ">
          <div class="flex flex-row">
            <div class="w-28 md:w-60 shrink-0">
              ...
              <div class="pt-6 text-gray-400">{{$t("episode.label.description")}}</div>
            </div>
            <div class="pl-12 grow-0">
          <div class="flex flex-row flex-grow justify-between">
            <div class="text-sm">{{$t("episode.label.duration") +" "+ duration() }}</div>
            <div class="text-sm mx-3">
              {{ $t("episode.label.pubdate") +" "+ new Date(episode.pubdate).toLocaleDateString() }}
            </div>
            <div class="text-sm">
              {{ $t("episode.label.filesize") +" "+ (episode.rawsize / (1024 * 1024)).toFixed(2) }}MB
            </div>
          </div>
