...
export default defineComponent({
  ...
  async setup(props, { emit }) {
    ...
    async function upload(server_path: string, fileObj: File, blob: Blob | undefined = undefined) {
      ...
      if (postData.body) {
        try {
          postResult = await $fetch( API_BASE + UPLOAD_AP, postData);
        } catch (err) {
    ...
    async function doTheSaving() {
      ...
      result = await $fetch( API_BASE + EPISODE_AP, postData);
      if (result.status != 201) {
        errors.value.push({ field: "", text: "saving" })
        return
      }
      if (serie_id != previousSeries_id && previousSeries_id > 0) {
        const slug = await $fetch( API_BASE + SERIE_AP, { method: "post", body: { id: previousSeries_id } })
    ...
    async function save(event) {
      ...
      var count: number = await $fetch( API_BASE + countUrl);
