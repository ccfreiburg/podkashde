...
async function post<T>(endpoint: string, data: T) {
  ...
  var postResult: Response = await $fetch( API_BASE + endpoint, postData);
...
async function fetchArchive(serverPath: string, slug: string, file: string): Promise<IFetchFileResult> {
  ...
  return await $fetch( API_BASE + FROMARCHIVE_AP, {
    method: "post",
    body: {
      serverPath,
      slug,
...
async function fetchFile(
  ...
  if (ContentFile.isQualifiedUrl(file)) {
    ...
    const ret = (await $fetch( API_BASE + FETCHLOCAL_AP, postData)) as IFetchFileResult;
    return ret;
  }
...
async function importPodcast(podcast: any) {
  ...
  $fetch( API_BASE + GENERATE_RSS_AP, { query: { slug: podkashde.slug } })
}
